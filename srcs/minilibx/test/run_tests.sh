...
log_vars(){
	logging "${LIGHT_GREEN}vars${RESET}" "$@"
}
log_error(){
...
at_exit() {
	status=$?
	[ $status -eq 0 ] && log_vars "Seem all went well" && exit 0
...
test_default_main(){
	...
	./mlx-test &
	PID="$!"
	log_vars "./mlx-test running in background, pid:" $PID
	...
	while [ $i -gt 0 ]; do
		...
		log_vars "countdown" $i
		sleep 1
		i=$((i - 1))
	done
	log_vars "Ready to \"just play\" using xdotool"
	wid1=$(xdotool search --name Title1)
	wid2=$(xdotool search --name Title2)
	wid3=$(xdotool search --name Title3)
	
	xdotool windowfocus $wid3
	log_vars "Focus Win3: Testing move mouse 100 100"
	xdotool mousemove 100 100
	log_vars "Focus Win3: Testing move mouse 200 200"
	xdotool mousemove 200 200
	log_vars "Focus Win3: Pressing escape to destroy window \"Win3\""
	xdotool key Escape
	
	log_vars "Focus Win2: Pressing escape to stop program"
